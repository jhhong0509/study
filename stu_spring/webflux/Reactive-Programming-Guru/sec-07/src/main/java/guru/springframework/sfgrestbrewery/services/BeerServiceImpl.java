...
public class BeerServiceImpl implements BeerService {
    ...
    @Override
    public Mono<BeerDto> updateBeer(Integer beerId, Mono<BeerDto> beerDto) {
        return beerRepository.findById(beerId)
                .zipWith(beerDto)
                .flatMap(objects -> Mono.just(objects.getT1().updateEntity(objects.getT2())))
                .flatMap(beer -> {
                    if(beer.getId() != null) {
    ...
    @Override
    public Mono<Void> reactiveDeleteBeer(Integer beerId) {
        return beerRepository.findById(beerId)
