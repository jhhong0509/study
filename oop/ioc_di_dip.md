...
IoC의 핵심은 **`언제`와 `무엇을`을 분리**하고 **`언제`와  `무엇을`이 서로 최대한 모르게 하는 것**이다.
...
로봇을 만드는 기계가 있다고 가정해보자.

이 기계는 로봇과 매우 강하게 결합되어 있어, 다른 무엇도 만들 수 없기 때문에 다른 로봇이 필요하다면, 기계를 새로 사야 한다.

<br>

그렇다면 로봇을 만드는 기계가 아닌 사용자의 모델링대로 출력해주는 3D 프린터가 있다고 가정해보자.

사용자가 모델링만 제대로 해서 넣어주게 되면, 그대로 만들어서 출력해 준다.

<br>

기존의 로봇을 만드는 기계는 해당 로봇에게 강하게 결합되어 있다고 할 수 있다.

하지만 3D 프린터를 사용함으로써 **무엇을 만들지에 대한 제어권이 사용자에게 넘어갔다.**

사용자는 어떤 과정을 거쳐서 내 모델링을 만드는지 전혀 관심이 없다.

단지 사용자의 모델링대로 출력해 줄 것이라고 믿을 뿐이다.

<br>

또한 구현체가 아닌 인터페이스에 의존하게 강제하는 규약이기도 하다.

우리는 3D 프린터가 원하는 인터페이스를 구현하는 모델링을 해야한다.
